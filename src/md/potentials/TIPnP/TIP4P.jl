...
Based on:
Abascal, Jose LF, and Carlos Vega. "A general purpose model for the condensed
phases of water: TIP4P/2005." The Journal of chemical physics 123.23 (2005).

Link:
https://pubs.aip.org/aip/jcp/article/123/23/234505/965459
"""
function TIP4P(bonds::T; constraints=nothing) where T

  shake = SHAKE(bonds, 0.9419)

  #TODO:
  #   - Add RATTLE
  #   - Allow for other bond constraints

  if constraints == nothing
    return Calculator(TIP4P; EF=TIP4P!, constraints=[shake])
  else
    push!(constraints, shake)
  end

  Calculator(TIP4P; EF=TIP4P!, constraints=constraints)
end
...
TIP4P(bdys::Union{MyCell, Vector{MyAtoms}}) = _TIP4P_PotVars(
  8.03e-3,    # eV
  3.1644,     # \AA
  0.1546,    # \AA
